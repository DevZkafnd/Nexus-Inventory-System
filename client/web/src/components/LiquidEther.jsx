...
export default function LiquidEther({
  ...
  useEffect(() => {
    // if (!mountRef.current) return; // Commented out to prevent error if ref is not attached yet, logic moved below
    ...
    class MouseClass {
      ...
      update() {
        ...
        this.coords_old.copy(this.coords);
        // if (this.coords_old.x === 0 && this.coords_old.y === 0) this.diff.set(0, 0); // Removed to allow movement from center
    ...
    class AutoDriver {
      ...
      update() {
        ...
        // Placeholder completion for cut-off code
...
`;
    ...
    // Output premultiplied color with strong alpha boost for visibility on white
    float alpha = smoothstep(0.0, 0.1, lenv); // Saturate alpha quickly
    vec3 outRGB = c * alpha;
...
`;
    ...
    class Divergence extends ShaderPass { 
     ...
     class Simulation {
      constructor(options) {
        ...
        this.autoDriver = new AutoDriver(Mouse, Common, {
            enabled: autoDemo,
            speed: autoSpeed,
      ...
      update() {
        ...
        if (this.options.isViscous) {
          velocity = this.viscous.update({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt });
        }
    ...
    class Output {
      ...
      update() {
        ...
        uniforms.velocity.value = this.simulation.fbos.vel_0.texture;
        Common.renderer.setRenderTarget(null);
        Common.renderer.render(this.scene, this.camera);
      }
    }
    ...
    class WebGLManager {
      constructor() {
        ...
        this.autoDriver = this.output.simulation.autoDriver;
        this.raf = null;
      ...
      dispose() {
        ...
        Mouse.dispose();
        if (Common.renderer && Common.renderer.domElement && Common.container) {
          try {
